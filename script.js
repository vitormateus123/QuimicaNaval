document.addEventListener("DOMContentLoaded", function () {
  const elementos = document.querySelectorAll(
    ".nao-metais, .metais-alcalinos, .metais-alcalino-terrosos, .semimetais, .halogenios, .gases-nobres, .outros-metais, .metais-de-transicao, .lantanideos, .actinideos"
  );
  ...
  function atualizarSelecao() {
    ...
    selecionadosNomes.textContent = selecionado
      ? selecionado.querySelector("small").textContent
      : "";
  }
  ...
  botaoConfirmar.addEventListener("click", function (event) {
    ...
    if (selecionado) {
      const idElemento = selecionado.getAttribute("idDica");
    ...
    if (!period && !group && !valence) {
      ...
      const element = valence
        ...
        : document.querySelector(
            `td[data-period='${period}'][data-group='${group}']`
          );
      ...
      if (element) {
        ...
        errorMessage.textContent =
          "Nenhum elemento encontrado com os dados fornecidos.";
      }
    }
  ...
  document
    .getElementById("search-element")
    .addEventListener("click", searchElement);

  document
    .querySelectorAll("#input-period, #input-group, #input-valence")
    .forEach((input) => {
